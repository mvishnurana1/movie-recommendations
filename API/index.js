...
app.get("/recommendations", async (req, res) => {
  ...
  const hindiFilms = await fetchForeignFilms(
    releaseDateLte,
    releaseDateGte,
    genre_code,
    "hi"
  );
  const germanFilms = await fetchForeignFilms(
    releaseDateLte,
    releaseDateGte,
    genre_code,
    "de"
  );
  const japaneseFilms = await fetchForeignFilms(
    releaseDateLte,
    releaseDateGte,
    genre_code,
    "ja"
  );
  const spanishFilms = await fetchForeignFilms(
    releaseDateLte,
    releaseDateGte,
    genre_code,
    "es"
  );
  const koreanFilms = await fetchForeignFilms(
    releaseDateLte,
    releaseDateGte,
    genre_code,
    "ko"
  );
  const englishFilms = await fetchForeignFilms(
    releaseDateLte,
    releaseDateGte,
    genre_code,
    "en"
  );
  const russianFilms = await fetchForeignFilms(
    releaseDateLte,
    releaseDateGte,
    genre_code,
    "ru"
...
async function fetchForeignFilms(
  releaseDateLte,
  releaseDateGte,
  genre_code,
  lang
...
function getUrlWithArgs(releaseDateLte, releaseDateGte, genre_code, lang) {
  const url = `${moviesEndPoint}?api_key=${secrets.api_key}&include_video=true&sort_by=vote_average.desc
